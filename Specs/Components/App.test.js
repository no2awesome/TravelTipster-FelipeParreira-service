...
import MessageForm from '../../client/src/Components/MessageForm/MessageForm.jsx'; // eslint-disable-line no-unused-vars
...
describe('Message Form Component', () => {
  const wrapper = shallow(<MessageForm initialState={{}}
    currentUser={currentUser}
    user={questionData[0].AnswersUsers[0]} closeForm={() => {}}/>);

  it('should contain a background element', () => {
    const background = wrapper.find('.background');
    expect(background).toBeDefined();
    expect(background.length).toBe(1);
  });

  it('should contain an icon to close the floating window', () => {
    const icon = wrapper.find('.fa-times');
    expect(icon).toBeDefined();
    expect(icon.length).toBe(1);
  });
  ...
  it('should contain a mini profile for the sender', () => {
    const miniProfile = wrapper.find('.mini-profile');
    expect(miniProfile).toBeDefined();
